...
async def settle_own_document(
    ...
    settle_id = (
        await session.execute(
            text("SELECT id FROM party_ledger_entry WHERE org_id=:o AND source_doc_type=:t "
                 "AND source_doc_id=:i ORDER BY id LIMIT 1"),
    ...
    if settle_id is None or against_id is None:
        return []
    ...
    return await allocate(
        session, org_id, settle_entry_id=settle_id, targets=[(against_id, free)]
    )
    ...
    rows = (
        await session.execute(
            text(
                ...
                "       se.effective_date, pv.doc_no, pv.voucher_type, pt.name AS payment_type "
